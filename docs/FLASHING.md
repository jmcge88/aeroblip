...
- Single core: the network task and UI loop share one CPU. Watch the heap
  ...
  The stock websocket library kept three copies of every incoming frame,
  so the full BNE airport board (~44 KB) aborted the C6 with `bad_alloc` on
  every connect; the vendored copy in `esp32/lib/ArduinoWebsockets` moves
  the payload instead (one copy), and devices also ask the server for
  `rows=10` so the board frame is a few KB on servers ≥ 0.10.5. A 44 KB
  frame from an older server now goes through with ~60 KB to spare.
...
| C6 unit: `abort()` right after `[ws] connected`, 3 times, then rollback, then OTA, forever | out of memory receiving a websocket frame (firmware < 0.10.6 copied each frame three times; the full airport board is ~44 KB) | USB-flash ≥ 0.10.6; upgrade the server to ≥ 0.10.5 so it honours the device's `rows=` cap as well |
